...
class UserService {
  public async updateConfig(selectedColorObj: selectedColorObj, userId: string) {
    try {
      if (isEmpty(selectedColorObj)) return { statusCode: 400, message: `object not provided` };

      const updateUserById = await DI.userRepository.findOne({ id: userId });
      console.log('updateUserById24', updateUserById);

      if (!updateUserById) return { statusCode: 400, message: `Provided user not found` };
      if (updateUserById.id !== userId) return { statusCode: 400, message: `Data does not belong to this user` };
      ...
      if (!updateUserById) return { statusCode: 400, message: `data does not exist` };
  ...
  public async getUserData(userId: string) {
    try {
      if (isEmpty(userId)) return { statusCode: 400, message: `Id not provided` };
  ...
  public async uploadDataToDb(userId, s3: S3, originalKey,imageName) {
    ...
    try {
      let response = await axios.post('https://35bbmbbmb2.execute-api.us-west-2.amazonaws.com/staging', {
        url: originalUrl,
      })
      ...
      let finalArr = [`${Math.round((data?.[0]/360)*1000)}°,${Math.round((data?.[1]/255)*100)}%,${Math.round((data?.[2]/255)*100)}%`]
      console.log("finalArrrrrrrrrrrrr",finalArr);
      let newHistory = { id: uuidv4(), hsv: finalArr,imageName:imageName };
      let newConfig = { ...findUser.config, history: [...findUser.config.history, newHistory], selectedColor: newHistory };
      

      console.log('newConfig in upload to Db', newConfig);

      const updatedVal = wrap(findUser).assign({ ...findUser, profilePicture: originalUrl, config: newConfig });
  ...
  public async deleteProfilePhoto(userId: string) {
    try {
      ...
      const updateUserById = await DI.userRepository.findOne({ id: userId });
      if (updateUserById.id !== userId) return { statusCode: 400, message: `something went wrong` };

      let updatedPhotoValue = wrap(updateUserById).assign({ ...updateUserById, profilePicture: '' });
  ...
  public async uploadToS3(s3: S3, fileData?: Express.Multer.File) {
    try {
      if (fileData.mimetype === 'text/csv') {
        return { success: false, message: 'Image should be in format jpg,jpeg,png' };
      }
      ...
      try {
        ...
        return { success: true, message: 'File Uploaded  Successfull', data: params.Key,imageName:fileData.originalname };
      } catch (error) {
