...
class AuthService {
  public async signup(userData: UserInput) {
    try {
      if (isEmpty(userData))
        return { statusCode: 400, message: `provided data is empty` };
      ...
      const createUserData: User = DI.userRepository.create({
        ...userData,
        id: uuidv4(),
        strategy: "local",
        config: configData,
  ...
  public async createUserForStrategy(userData: User, photoId) {
    try {
      ...
      if (findUser && findUser.strategy !== userData.strategy)
        return { statusCode: 400, message: `Please login with other method` };
  ...
  public async login(userData: User) {
    try {
      ...
      if (!findUser)
        return { statusCode: 404, message: `Incorrect email or password` };

      const isPasswordMatching: boolean =
        userData.password === findUser.password;

      if (!isPasswordMatching)
        return { statusCode: 400, message: `Incorrect email or password` };
